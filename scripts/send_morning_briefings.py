...
def _format_sms(d: dict) -> str:
    ...
    if d["urgent"]:
        lines.append(f"🚨 {d['urgent'][0]}")
    if d["actions"]:
        lines.append(f"▶ {d['actions'][0]}")
...
def _format_slack(d: dict) -> dict:
    ...
    blocks = [
        ...
        {"type": "section", "fields": [
            ...
            {"type": "mrkdwn", "text": f"📈 *MRR:* ${d['mrr']:,}/wk ({'+' if d['mrr_change_pct']>0 else ''}{d['mrr_change_pct']:.1f}%)"},
            {"type": "mrkdwn", "text": f"📊 *Gross Margin:* {d['gross_margin_pct']:.0f}%"},
        ]},
    ]
    if d["urgent"]:
        urgent_text = "\n".join(f"• {u}" for u in d["urgent"])
        blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": f"🚨 *Urgent Alerts:*\n{urgent_text}"}})
    if d["good_news"]:
        good_text = "\n".join(f"• {g}" for g in d["good_news"])
        blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": f"✅ *Good News:*\n{good_text}"}})
    if d["actions"]:
        action_text = "\n".join(f"{i+1}. {a}" for i, a in enumerate(d["actions"]))
        blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": f"📋 *Today's Actions:*\n{action_text}"}})
    blocks.append({"type": "divider"})
    blocks.append({"type": "context", "elements": [
        {"type": "mrkdwn", "text": f"Week of {d['week_start']} · Weekly CFO Briefing · ~$0.003/call"}
    ]})
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f5f5f7;margin:0;padding:32px">
  <div style="max-width:600px;margin:0 auto;background:white;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08)">
    <div style="background:linear-gradient(135deg,#f59e0b,#ea580c);padding:24px 32px">
      <div style="color:white;font-size:22px;font-weight:900">🌅 Morning CFO Briefing</div>
    ...
    <div style="padding:24px 32px">
      ...
      {'<h3 style="color:#ef4444;font-size:13px;margin:0 0 8px">🚨 Urgent Alerts</h3><ul style="margin:0 0 20px;padding-left:20px;font-size:13px;color:#374151">' + urgent_html + '</ul>' if urgent_html else ''}
      {'<h3 style="color:#16a34a;font-size:13px;margin:0 0 8px">✅ Good News</h3><ul style="margin:0 0 20px;padding-left:20px;font-size:13px;color:#374151">' + good_html + '</ul>' if good_html else ''}
      {'<h3 style="color:#2563eb;font-size:13px;margin:0 0 8px">📋 Today\'s Actions</h3><ol style="margin:0 0 20px;padding-left:20px;font-size:13px;color:#374151">' + action_html + '</ol>' if action_html else ''}
      <div style="font-size:11px;color:#9ca3af;border-top:1px solid #e5e7eb;padding-top:16px;margin-top:8px">
        Week of {d['week_start']} · AI CFO Agent · ~$0.003/briefing
      </div>
    </div>
  </div>
</body>
...
async def _send_email(to: str, subject: str, html: str) -> None:
    ...
    if resp.status_code not in (200, 202):
        print(f"  ✗  SendGrid error {resp.status_code}: {resp.text[:120]}")
    else:
...
async def _send_slack(webhook_url: str, payload: dict) -> None:
    ...
    if resp.status_code != 200:
        print(f"  ✗  Slack error {resp.status_code}: {resp.text[:80]}")
    else:
...
async def _send_sms(to: str, body: str) -> None:
    ...
    if resp.status_code not in (200, 201):
        print(f"  ✗  Twilio error {resp.status_code}: {resp.text[:120]}")
    else:
...
async def send_briefing(
    ...
    if resp.status_code != 200:
        ...
        return
    d = resp.json()
    print(f"  ✓  Briefing generated for {d['company_name']} | runway={d['runway_months']:.1f}mo")
...
async def main() -> None:
    ...
    print(f"→ Sending morning briefing for run {args.run_id}…")
    await send_briefing(
        run_id=args.run_id,
